    ...
    fn default() -> Self {
        Config {
            ...
            screens: vec![Screen {
                ...
                aliases: vec![],
                position: GridPosition { x: 0, y: 0, w: 160, h: 120 },
            }],
            links: vec![],
...
impl Config {
    ...
    pub fn save(&self, path: &Path) -> Result<(), Error> {
        ...
        fs::write(&tmp, body).map_err(|source| Error::Io { path: tmp.clone(), source })?;
        fs::rename(&tmp, path).map_err(|source| Error::Io { path: path.into(), source })?;
        Ok(())
    }
    ...
    pub fn validate(&self) -> Result<(), Error> {
        ...
        for l in &self.links {
            if !self.screens.iter().any(|s| s.name == l.from) {
                return Err(Error::Invalid(format!("link.from {:?} is not a screen", l.from)));
            }
            if !self.screens.iter().any(|s| s.name == l.to) {
                return Err(Error::Invalid(format!("link.to {:?} is not a screen", l.to)));
            }
            if l.from == l.to {
    ...
    pub fn generate_synergy_conf(&self) -> String {
        ...
        let _ = writeln!(out, "# Generated by Synbad. DO NOT EDIT — regenerated from config.toml.");
        let _ = writeln!(out);
        ...
        if !self.links.is_empty() {
            ...
            for l in &self.links {
                ...
                by_from.entry(&l.to).or_default().push((l.side.opposite(), &l.from));
            }
        ...
        Config {
            ...
            screens: vec![
                ...
                Screen {
                    name: "beta".into(),
                    aliases: vec![],
                    position: GridPosition { x: 160, y: 0, w: 160, h: 120 },
                },
            ],
            links: vec![Link {
                from: "alpha".into(),
